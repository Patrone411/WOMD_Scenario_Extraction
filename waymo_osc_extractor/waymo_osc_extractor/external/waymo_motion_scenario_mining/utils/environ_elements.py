...
def _safe_polygon_from_coords(coords: np.ndarray, default_width: float = 1.0):
    """
    Build a robust geometry from a set of coords:
    - >=3 unique points -> Polygon (fixed with buffer(0) if needed)
    - 2 points -> buffered LineString
    - 1 point -> buffered Point
    - 0 points -> None
    """
    if coords is None or len(coords) == 0:
        return None

    # Drop rows with NaNs/Infs and remove duplicate rows
    ...
    # Unique points (order doesn't matter for a polygon shell here)
    ...
    if n >= 3:
        try:
            poly = Polygon(coords)
            # Fix common invalids (self-intersections, tiny bow-ties)
            if not poly.is_valid or poly.is_empty:
                poly = poly.buffer(0)
            if not poly.is_empty:
                return poly
        except Exception:
            pass  # fall through to buffered primitives

    if n == 2:
        ls = LineString(coords)
        geom = ls.buffer(default_width / 2.0)
        return geom if not geom.is_empty else None

    if n == 1:
        pt = Point(coords[0])
        geom = pt.buffer(default_width / 2.0)
        return geom if not geom.is_empty else None
...
class EnvironmentElementsWaymo:
    ...
    def __create_other_object_polygon_set(self, roadgraph_type, roadgraph_xyz, roadgraph_lane_id):
        for key in self.other_object_type:
            ...
            other_object_pts = roadgraph_xyz[other_object_mask, :2]
            other_object_id_list = roadgraph_lane_id[other_object_mask]
            other_object_unique_id = np.unique(other_object_id_list)

            # pick a sensible default width (tweak if you know typical sizes)
            default_width = 2.0 if key == 'cross_walk' else 0.6

            for other_object_id in other_object_unique_id:
                coords = other_object_pts[np.where(other_object_id_list == other_object_id)[0], :]
                geom = _safe_polygon_from_coords(coords, default_width=default_width)
                if geom is None:
                    continue  # not enough info to build a geometry; skip
                self.other_object[key].append(geom)
